...
export default function WLCTracker() {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("wlc-data");
      if (stored) setData(JSON.parse(stored));
    } catch {}
    try {
      const w = localStorage.getItem("wlc-weight");
  ...
  const save = useCallback((newData) => {
    setData(newData);
    try { localStorage.setItem("wlc-data", JSON.stringify(newData)); } catch {}
  }, []);
  ...
  const saveWeight = () => {
    ...
    if (w > 0) {
      setWeight(w);
      try { localStorage.setItem("wlc-weight", String(w)); } catch {}
    }
  };
        ...
        <div style={{ textAlign: "center", marginTop: 32 }}>
          <button onClick={() => {
            if (confirm("Reset all data? This cannot be undone.")) {
              setData({});
              try { localStorage.removeItem("wlc-data"); } catch {}
            }
