...
## [0.18.0 → 0.18.2] — 23 Apr 2026 — Update toast summary + shareable changelog links
...
- **Changelog deep links now work for people who aren&rsquo;t signed in (0.18.2).** Previously a recipient clicking a shared `/changelog#X.Y.Z` link would hit the sign-in screen first, and after signing in with Google the URL fragment was gone — they&rsquo;d land on the check-in page with no way to find the version you were trying to show them. Changelog is now a public route so it opens directly, no account required. The &ldquo;Close&rdquo; button falls back to home when there&rsquo;s no previous page to return to. `/health` is now public for the same reason.
...
- **Log any sleep duration, not just 5&ndash;10 hours.** The Sleep card on Check In used to cap out at 5 hours as the lowest preset — which was a problem for any rough night that dipped below. Now there's a **Custom&hellip;** button beside the preset row. Tap it, type the exact hours (anything 0&ndash;24, half-hour increments), hit Save. Three hours of sleep? Nine and a half? Zero because you forgot? All logs correctly now. Preset buttons stay for the common values — the custom input is an addition, not a replacement.
- **Share your reflexion journal with specific people.** New toggle under **My Preferences &rarr; Sharing**. Pick exactly who sees your written reflections — one person, two people, or turn on the &ldquo;everyone&rdquo; switch if that&rsquo;s simpler. Defaults to <strong>off</strong>; nothing changes unless you flip a toggle.
...
- **`src/pages/MyPreferences.jsx`** — new &ldquo;Sharing&rdquo; section sitting between Notifications and Targets. Each scope (journal / wellness) renders a &ldquo;Share with all active users&rdquo; toggle (persists the boolean via the existing `updateProfile` path) and, when the all-toggle is off, a per-person checkbox list driven by `listShareableProfiles()`. Checkbox toggles upsert/delete rows in `entry_shares`. 9 new tests cover rendering, the reflected initial state, all-toggle persistence, per-person add/remove, and the &ldquo;hide list when all is on&rdquo; rule.
- **`src/components/OwnerSelector.jsx` (new)** — `<select>` dropdown parameterised by `scope`; queries the relevant `shared_{scope}_entries` view for distinct owners, prepends a &ldquo;Me&rdquo; option, hides itself via `display:none` when no sharers exist so self-only users see no chrome. 7 tests pin the distinct-sharer logic, self exclusion, onChange wiring, and error-path fallback.
